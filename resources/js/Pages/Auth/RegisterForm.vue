<template>
    ...
    <div class="flex min-h-screen flex-col justify-center py-12 sm:px-6 lg:px-8">
        ...
        <div class="mt-8 mx-4 sm:mx-auto sm:w-full sm:max-w-md">
            <div class="bg-white py-8 px-4 shadow rounded-lg sm:px-10">
                <form
                    class="space-y-6"
                    @submit.prevent="form.post(registerStoreRoute)"
                >
                    ...
                    <div class="rounded-md border border-slate-300 px-3 py-2 shadow-sm focus-within:border-slate-600 focus-within:ring-1 focus-within:ring-slate-600">
                        ...
                        <input
                            ...
                            id="name"
                            class="block w-full border-0 p-0 text-slate-900 placeholder-slate-500 focus:ring-0 sm:text-sm"
                            placeholder="ชื่อเล่นหรือชื่อใดก็ได้ตามต้องการ"
                            v-model="form.name"
                        >
                    </div>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            <input
                                id="terms_and_policies_accepted"
                                name="terms_and_policies_accepted"
                                type="checkbox"
                                class="h-4 w-4 rounded border-slate-300 text-slate-600 focus:ring-slate-500"
                                v-model="form.terms_and_policies_accepted"
                            >
                            <label
                                for="terms_and_policies_accepted"
                    ...
                    <div>
                        <button
                            type="submit"
                            :disabled="!form.name || !form.terms_and_policies_accepted"
                            class="flex w-full justify-center rounded-md border border-transparent bg-slate-600 py-2 px-4 text-sm font-semibold text-white shadow-sm hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-slate-500 focus:ring-offset-2 disabled:bg-slate-400 disabled:text-slate-100"
                        >
                            ยืนยัน
                        </button>
                    </div>
    ...
    <TermsAndPolicies ref="termsModal" />
</template>
